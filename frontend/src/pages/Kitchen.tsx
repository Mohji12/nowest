...
export default function Kitchen() {
    ...
    if (portfolioData && Array.isArray(portfolioData) && portfolioData.length > 0) {
      ...
      const kitchenFiltered = (portfolioData as any[])
        .filter((item: any) => {
          ...
          return kitchenKeywords.some(keyword => searchText.includes(keyword));
        })
        .map((item: any) => {
  ...
  const navigateImage = (direction: 'prev' | 'next') => {
    ...
    if (direction === 'prev') {
      setSelectedImageIndex(
        selectedImageIndex === 0 ? kitchenImages.length - 1 : selectedImageIndex - 1
      );
    } else {
      setSelectedImageIndex(
        selectedImageIndex === kitchenImages.length - 1 ? 0 : selectedImageIndex + 1
      );
    }
  };
  ...
  // Background image for kitchen
  const kitchenBackgroundImage = 'https://jgi-menteetrackers.s3.ap-south-1.amazonaws.com/Nowest_Image/Portrait_Kitchen_Perfect_Fit_NG_Roller_Bubbles_White.jpg.webp';
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative text-center mb-8 sm:mb-12 md:mb-16 overflow-hidden w-full" style={{ minHeight: '400px' }}>
        {/* Background image */}
        <img
          src={kitchenBackgroundImage}
          alt="Kitchen background"
          className="absolute inset-0 w-full h-full object-cover z-0"
          style={{ minHeight: '400px', width: '100%' }}
          onError={(e) => {
            console.error('Kitchen background image failed to load:', kitchenBackgroundImage);
      ...
      {selectedImageIndex !== null && (
        ...
        >
          ...
          >
            <img
              src={kitchenImages[selectedImageIndex]}
          ...
          <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 text-white text-xs sm:text-sm bg-black/50 px-3 py-1 rounded-full">
            {selectedImageIndex + 1} / {kitchenImages.length}
          </div>
        </div>
      )}
    </div>
